...
select 
sum(case when t.id is null then 1 else 0 end) as id ,
sum(case when t.title is null then 1 else 0 end) as title ,
sum(case when t.year is null then 1 else 0 end) as year ,
sum(case when t.date_published is null then 1 else 0 end) as date_published ,
sum(case when t.duration is null then 1 else 0 end) as duration ,
sum(case when t.country is null then 1 else 0 end) as country ,
sum(case when t.worlwide_gross_income is null then 1 else 0 end) as worlwide_gross_income ,
sum(case when t.languages is null then 1 else 0 end) as languages ,
sum(case when t.production_company is null then 1 else 0 end) as production_company 
from movie as t;
...
WHERE (country = "USA") AND (year = 2017) AND (EXTRACT(MONTH FROM date_published) = 03) AND (r.total_votes > 1000)
GROUP BY genre;
...
SELECT
	languages,
    total_votes AS number_of_votes
FROM
	movie as m
	INNER JOIN ratings AS r
	ON m.id = r.movie_id
WHERE
	languages IN ('German', 'Italian')
GROUP BY languages;
...
directors AS (
SELECT n.name AS director, COUNT(*) as movie_count
FROM movie AS m
...
ON m.id = g.movie_id
INNER JOIN top_three_genre AS tt
ON g.genre = tt.genre
...
SELECT 
    ...
    RANK() OVER (ORDER BY SUM(total_votes*avg_rating) / SUM(total_votes) DESC) AS actress_rank
FROM
movie as m
INNER JOIN
ratings as r
...
SELECT 
    ...
    SUM(m.duration) OVER (ORDER BY g.genre) AS running_total_duration,
    ROUND(AVG(m.duration) OVER (ORDER BY g.genre),2) AS moving_avg_duration	
FROM
movie as m
INNER JOIN
genre as g
...
GROUP BY m.title) AS  hg
WHERE hg.movie_rank<=5;
...
names as n
ON rm.name_id = n.id
WHERE (r.avg_rating >8) AND (rm.category = "actress") AND (g.genre LIKE '%Drama%')
GROUP BY n.name
LIMIT 3;
...
avginterd AS (
SELECT AVG(days_diff) AS avg_inter_movie_days , name  
FROM daysdiff
GROUP BY name
),
 top_directors AS (
